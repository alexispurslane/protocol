...
const generatedHeader = `// Copyright 2025 The Go Language Server Authors.
...
func generateFileSources(src []byte, structFiles map[string]string, defaultFile string) (map[string][]byte, error) {
	...
	receiverTypeName := func(expr ast.Expr) string {
		switch t := expr.(type) {
		case *ast.Ident:
			return t.Name
		case *ast.StarExpr:
	...
	splitGenDecl := func(decl *ast.GenDecl) []*ast.GenDecl {
		if len(decl.Specs) <= 1 {
	...
	structNameFromAssertion := func(spec *ast.ValueSpec) (string, bool) {
	...
	importOrder := []string{"fmt", "strings", "json", "jsontext"}

	filterComments := func(decls []ast.Decl) []*ast.CommentGroup {
...
func main() {
	...
	formatted, err := gofumpt.Source([]byte(generatedCode), gofumpt.Options{
		LangVersion: "go1.25",
		ExtraRules:  true,
	})
	if err != nil {
	...
	fileSources, err := generateFileSources(formatted, structFiles, "lsp.go")
	if err != nil {
		panic(err)
	}

	fileNames := make([]string, 0, len(fileSources))
